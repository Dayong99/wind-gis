...
def data_to_image(data_set, resolution, input_name: str, geo_range: list):
    ...
    for i in ch:
        ...
        if "Channel01" == i:
            ...
            channel_three = output
        if count>5:
            temps = output.copy()
            temps[::] = Imax
            output = temps-output
        output = cv2.equalizeHist(output.astype(np.uint8))
        img = Image.fromarray(output)
        img = img.convert("L")

        image = img.resize(projection.get_column_from_line(img.height, img.width, max_len))
...
def channel_to_color(save_name):
    ...
    B = channel_one[:, :]
    NIR = channel_three[:,:]
    likeG = 0.45*R + 0.1*NIR + 0.45*B
    b_out = cv2.equalizeHist(B.astype(np.uint8))
    g_out = cv2.equalizeHist(likeG.astype(np.uint8))
    r_out = cv2.equalizeHist(R.astype(np.uint8))
